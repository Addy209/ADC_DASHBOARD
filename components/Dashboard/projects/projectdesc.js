...
const SpecficProject=props=>{
    ...
    React.useEffect(()=>{
        if(props.id){
            ...
            client.request(projectQuery, {id:props.id}).then(resp=>{
    ...
    return (
        <div className={styles.project}>
            {values && values.live?null:<div>
        <Button onClick={()=>setEdit(!edit)} type="primary">{edit?"Close":"Edit"}</Button>
        </div>}
        {edit?<div>
            <Divider orientation="left">Edit Project</Divider>
                <CreateProject data={props} values={values} refetch={refetch} mode="update"/>
        </div>:
        values?<div>
        ...
        {values && values.live?null:<><Divider orientation="left">Upload Project Documents</Divider>
        <div className={styles.uploadform}>
            <DocumentUpload refetch={setDoc} id={props.id} />
        </div></>}

        <div>
        <Divider orientation="left">Uploaded Documents For This Project</Divider>
