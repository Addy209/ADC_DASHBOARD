...
const ExcelExport=props=>{
    ...
    let data=props.data.map((val, index)=>({
        ...
        finalpayment:val.finalPayment
            ...
            console.log(data)
    return (
        <CSVLink data={data} headers={headers} filename={`Expenditure Report-${new Date().toDateString()}.xls`}>
    <div className={styles.xlsbutton}>
    <Button size="large" type="primary">Download Report in Excel&nbsp; <SiMicrosoftexcel size="20px" style={{verticalAlign:"middle"}} /></Button><br/>
      </div>
      </CSVLink>
    )
}
